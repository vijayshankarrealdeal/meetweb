...
def genrate_token(key):
    token = jwt.encode({'id': key, 'exp': datetime.utcnow(
    ...
    try:
        ...
        for item in items:
            try:
                ...
                k['time'] = item.find_element_by_xpath(
                    './/div[2]//div[1]').text
                k['flight'] = item.find_element_by_xpath(
                    './/div[2]//div[2]').text
                k['airline'] = item.find_element_by_xpath(
                    './/div[2]//div[3]').text
                k['info_url'] = item.find_element_by_xpath(
                    './/div[2]').find_element_by_tag_name('a').get_attribute('href')
                k['status'] = item.find_element_by_xpath(
                    './/div[contains(@class ,"flight-col flight-col__status")]').text
                data.append(k)
            except:
                pass
...
def get_flights(orgin, destination, date, adults, children, infants):
    ...
    try:
        ...
        all_pages = pages.find_elements_by_xpath(
            './/span[@class = "page-num"]')
        for page in all_pages:
            cards = driver.find_elements_by_class_name(
                'c-flight-listing-row-v2')
            for card in cards:
                k = {}
                fight_img = card.find_element_by_xpath(
                    './/div[@class = "logo"]').find_element_by_tag_name('img').get_attribute('src')
                fight_name = card.find_element_by_xpath(
                    './/a[@class = "flight-name"]').text
                orgin_details = card.find_element_by_class_name('left-wing')
                orgin_code = orgin_details.find_element_by_class_name(
                    'airport-code').text
                orgin_date = orgin_details.find_element_by_class_name(
                    'date').text
                orgin_time = orgin_details.find_element_by_class_name(
                    'time').text
                orgin_city = orgin_details.find_element_by_xpath(
                    './/div[4]').text
                ####
                destination_details = card.find_element_by_class_name(
                    'right-wing')
                dest_code = destination_details.find_element_by_class_name(
                    'airport-code').text
                dest_date = destination_details.find_element_by_class_name(
                    'date').text
                dest_time = destination_details.find_element_by_class_name(
                    'time').text
                dest_city = destination_details.find_element_by_xpath(
                    './/div[4]').text
                ####
                duration_stops_div = card.find_element_by_class_name(
                    'flight-summary')
                duration_of_flight = duration_stops_div.find_element_by_xpath(
                    './/div[2]//div[1]//div[2]').text
                duration_stops = duration_stops_div.find_element_by_xpath(
                    './/div[2]//div[1]//div[6]').text

                flight_price = card.find_element_by_class_name(
                    'price-section').text
        ...
        df = df = pd.read_csv(
            f'{orgin.upper()}-{destination.upper()}.csv', index_col=0)
        ...
        for i in range(len(df.dest_date) - 1):
            ...
            if p != k:
                ...
                st = str(string_[0]) + '-' + str(string_[1]) + \
                    '-' + str(int(string_[2]) + 1)
...
def askquestions(question):
    ...
    headers = {'Content-Type': 'application/json',
               'Authorization': "eef56569-aa6d-44a4-8798-dd368debb2ab"}
